...
import { WEBFORMS_LOCATORS } from '../../locators/WebForms.locators';

/**
 * WebFormsPage - Page Object Model
 * 
 * Implements page interactions using centralized locators
 * Follows OOP principles and best practices
 */
export class WebFormsPage extends BasePage {
  
  // ===== Locator Getters =====
  // All locators use centralized definitions from locators/WebForms.locators.ts
  
  private get exampleInput(): Locator {
    return this.locatorHelper.getLocatorSync(WEBFORMS_LOCATORS.INPUTS.EXAMPLE_INPUT);
  }
  
  private get submitButton(): Locator {
    return this.locatorHelper.getLocatorSync(WEBFORMS_LOCATORS.BUTTONS.SUBMIT);
  }
  
  private get mainContainer(): Locator {
    return this.locatorHelper.getLocatorSync(WEBFORMS_LOCATORS.CONTAINERS.MAIN);
  }
