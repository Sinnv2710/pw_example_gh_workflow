...
import { WEBFORMS GRID OVERVIEW_LOCATORS } from '../../locators/WebForms Grid Overview.locators';

/**
 * WebForms Grid OverviewPage - Page Object Model
 * 
 * Implements page interactions using centralized locators
 * Follows OOP principles and best practices
 */
export class WebForms Grid OverviewPage extends BasePage {
  
  // ===== Locator Getters =====
  // All locators use centralized definitions from locators/WebForms Grid Overview.locators.ts
  
  private get exampleInput(): Locator {
    return this.locatorHelper.getLocatorSync(WEBFORMS GRID OVERVIEW_LOCATORS.INPUTS.EXAMPLE_INPUT);
  }
  
  private get submitButton(): Locator {
    return this.locatorHelper.getLocatorSync(WEBFORMS GRID OVERVIEW_LOCATORS.BUTTONS.SUBMIT);
  }
  
  private get mainContainer(): Locator {
    return this.locatorHelper.getLocatorSync(WEBFORMS GRID OVERVIEW_LOCATORS.CONTAINERS.MAIN);
  }
